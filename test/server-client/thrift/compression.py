...
def expand_dim(flat_arr, graph_shape):
    ...
    for shape in graph_shape:
        length = shape[0] * shape[1] if len(shape) == 2 else shape[0]
        expand_arr = np.reshape(flat_arr[start_idx:start_idx+length], shape) if len(shape) == 2 else flat_arr[start_idx:start_idx+length]
